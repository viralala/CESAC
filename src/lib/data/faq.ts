...
/**
 * The questions, with Attack on Token's form and fee filled in from whatever
 * organisers last saved (see event-content.ts), so changing either on the
 * console does not leave this page quoting the old one.
 */
export function faqFor(register: { formUrl: string; amountInr: number }): readonly FaqGroup[] {
  return [
    {
      id: "cesac",
      title: "CESAC",
...
export const FAQ: readonly FaqGroup[] = faqFor(REGISTER);
